...
const SNEAKER_COLUMNS = [
  ...
  'quantity', 'has_invoice', 'cashback_status'
]
...
// Colonnes valides pour la table accounts
const ACCOUNT_COLUMNS = ['user_id', 'group_name', 'site', 'email', 'password', 'notes']
...
export async function updateAccount(userId, accountId, updates) {
  ...
  try {
    ...
    const res = await fetch('/api/data/accounts', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: accountId, ...row }),
      credentials: 'same-origin'
    })
