...
const SNEAKER_COLUMNS = [
  'user_id', 'name', 'brand', 'sku', 'size', 'category',
  'buy_price', 'sell_price', 'fees', 'status', 'image_url',
  'buy_date', 'sell_date', 'source', 'buyer_name', 'notes',
...
function sanitizeForSupabase(row) {
  ...
  for (const col of DATE_COLUMNS) {
    if (result[col] === '' || result[col] === undefined) {
      result[col] = null
    }
  }
  return result
